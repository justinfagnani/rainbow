...
export type PatternMatches = {[key: number]: string|Pattern|Pattern[]};
...
export interface Options {
  patterns: Patterns;
  inheritenceMap: {[name: string]: string};
...
class Prism {
  ...
  refract?: (code: string, language: string, patterns?: Pattern[]) => string;
  ...
  constructor(options: Options) {
     ...
     */
    const replacements:
        {[position: number]: {replace: string, with: string}} = {};
     ...
     */
    const replacementPositions: {[startPos: number]: number} = {};
     ...
     *
     * @param start    start position of new match
     * @param end      end position of new match
     */
    function _matchIsInsideOtherMatch(start: number, end: number) {
      for (const keyStr in replacementPositions) {
     ...
     *
     * @param name        name of the pattern (ie keyword.regex)
     * @param code        block of code to wrap
     */
    function _wrapCodeInSpan(name: string, code: string): string {
     ...
     * the markup
     *
     * @param code         the code to process replacements in
     */
    function _processReplacements(code: string): string {
      const positions = keys(replacements);
      for (const position of positions) {
        const replacement = replacements[position];
        code = replaceAtPosition(position, replacement.replace,
     ...
     * bit.
     */
    function _cloneRegex(regex: RegExp): RegExp {
      let flags = '';
     ...
     * be called directly.
     */
    function _processPattern(pattern: Pattern, code: string, offset = 0): false|
        {remaining : string, offset : number} {
      let regex: RegExp = pattern.pattern!;
      if (regex === undefined) {
        return false;
      }
      ...
      if (!pattern.name && pattern.matches &&
          typeof (pattern.matches as PatternMatches)[0] === 'string') {
        pattern.name = (pattern.matches as PatternMatches)[0] as string;
        delete (pattern.matches as PatternMatches)[0];
      }
       ...
       */
      function onMatchSuccess(repl: string) {
        ...
        // For debugging
        // console.log('Replace ' + match[0] + ' with ' + replacement + ' at
        // position ' + startPos + ' to ' + endPos);

        // Store what needs to be replaced with what at this position
        replacements[startPos] = {'replace' : match[0], 'with' : repl};
       ...
       *
       * @param groupKey      index of group
       */
      function _processGroup(groupKey: number) {
        ...
        const group = (pattern.matches as PatternMatches)[groupKey] as Pattern;
         ...
         * a span with a name
         */
        const _getReplacement = function(
            passedBlock: string, replaceBlock: string, matchName?: string|0) {
          replacement = replaceAtPosition(
              indexOfGroup(match, groupKey), passedBlock,
              matchName ? _wrapCodeInSpan(matchName, replaceBlock)
                        : replaceBlock,
              replacement);
          return;
        };
        ...
        localCode = prism.refract!(block, currentLanguage,
                                   ((groupToProcess as []).length
                                        ? groupToProcess
                                        : [ groupToProcess ]) as Pattern[]);
     ...
     */
    function _processCodeWithPatterns(code: string,
     ...
     */
    function getPatternsForLanguage(language: string) {
     ...
     * specified
     *
     * @param patterns optionally specify a list of patterns
     */
    function _highlightBlockForLanguage(code: string, language: string,
                                        patterns?: Pattern[]): string {
